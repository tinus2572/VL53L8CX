...
fn write_results(tx: &mut Tx<USART2>, results: &ResultsData, width: usize) {
    ...
    writeln!(tx, "VL53L8A1 Double Target Detection application\n").unwrap();
    writeln!(tx, "Cell Format :\n").unwrap();
    writeln!(
        tx, 
        "\x1b[96m{dis:>20}\x1b[0m \x1b[96m{dis2:<20}\x1b[0m", 
        dis="Distance [mm]", 
        dis2="Distance (target 2)"
    ).unwrap();
    writeln!(
        tx, 
        "\x1b[93m{sig:>20}\x1b[0m \x1b[93m{sig2:<20}\x1b[0m", 
        sig="Signal [kcps/spad]", 
        sig2="Signal (target 2)"
    ).unwrap();

    for j in (0..(width*2)).step_by(2) {
        for _ in 0..width { write!(tx, "+-----------").unwrap(); } writeln!(tx, "+").unwrap();
        for i in (0..(width*2)).step_by(2) {
            write!(
                tx, 
                "|\x1b[96m{dis:>5}\x1b[0m \x1b[96m{dis2:<5}\x1b[0m", 
            dis=results.distance_mm[width*j+i], 
            dis2=results.distance_mm[width*j+i+1]
            ).unwrap();
        } write!(tx, "|\n").unwrap();

        for i in (0..(width*2)).step_by(2) {
            let mut sig: u32 = results.signal_per_spad[width*j+i];
            let mut sig2: u32 = results.signal_per_spad[width*j+i+1];
            if sig > 9999 { sig = 9999; }
            if sig2 > 9999 { sig2 = 9999; }
            write!(
                tx, 
                "|\x1b[93m{sig:>5}\x1b[0m \x1b[93m{sig2:<5}\x1b[0m", 
                sig=sig, 
                sig2=sig2
            ).unwrap();
        } write!(tx, "|\n").unwrap();
